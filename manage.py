...
# Direct import of package.py to avoid triggering hello __init__ during the build process
spec = importlib.util.spec_from_file_location(
    "package",
    os.path.join(os.path.dirname(__file__), "hello", "package.py")
)
...
if sys.argv[-1] == 'dry-run':
    ...
    if branch != "master":
        sys.exit("dry-run from a branch other than master is disallowed.")
    os.system("pip uninstall -y hello")
    os.system("rm -rf requirements.txt")
    write_requirements()
    os.system("rm -rf hello.egg-info")
...
if sys.argv[-1] == 'publish':
    ...
    os.system("pip install -e .")
    os.system("git tag -a %s -m 'version %s'" % ("hello-" + version, "hello-" + version))
    os.system("git push")
